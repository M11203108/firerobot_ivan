...
qos_latest_only = QoSProfile(
    history=HistoryPolicy.KEEP_LAST,
    depth=1,
    reliability=ReliabilityPolicy.BEST_EFFORT,   # 要絕對到就換 RELIABLE
    durability=DurabilityPolicy.VOLATILE         # 不保留
)
...
class SimpleFireMission(Node):
    ...
    def wait_until_hot(self):
        ...
        while rclpy.ok():
            ...
            v = self.latest_temp
            if self.armed and v is not None and float(v) >= TEMPERATURE_THRESHOLD:
                self.armed = False            # 本輪只觸發一次
                self.latest_temp = None       # 清掉舊值，避免 re-arm 後立刻再觸發
                self.get_logger().info(f"🔥 溫度達標 {float(v):.1f} °C，開始任務")
                return True
...
def main(args=None):
    ...
    try:
        ...
        # 只在 context 還有效時關閉；避免「關兩次」的 RuntimeError
        try:
            if rclpy.ok():
                rclpy.shutdown()
        except RuntimeError:
