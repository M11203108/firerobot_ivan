...
def main():
    ...
    # 進入角度控制模式
    current_angle = 0  # 當前角度（以 0 度為基準）
    while True:
        user_input = input("Enter target angle (e.g., 20), or 'q' to quit: ").strip()
        ...
        try:
            ...
            # 檢查目標角度是否在合法範圍內
            if not (0.0 <= target_angle <= up_limit_angle):
                print(f"Invalid angle. Target angle must be within 0.00 to {up_limit_angle:.2f} degrees.")
                continue
            ...
            while True:
                ...
                move_motor(client, UNIT, direction, target_circle, 200)
