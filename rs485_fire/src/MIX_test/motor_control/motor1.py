...
class MotorControllerupdo:
    ...
    def move_to_angle(self, zero_encoder, target_angle, timeout=1):
        """移動到指定角度（加上超時機制）"""
        try:
            current_encoder = self.read_encoder_raw()
            current_angle = (current_encoder - zero_encoder) * 360.0 / self.resolution
            ...
            if abs(angle_diff) < 1.0:
                print(f"Motor {self.unit} already at target angle: {target_angle:.2f}")
                return
            ...
            while True:
                ...
                if time.time() - start_time > timeout:
                    print(f"[WARN] Timeout moving to {target_angle:.2f} degrees after {timeout}s.")
                    break
                time.sleep(0.05)
        except Exception as e:
            print(f"[ERROR] Failed to move to target angle: {e}")
