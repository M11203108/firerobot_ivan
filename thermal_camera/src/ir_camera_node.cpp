...
// ── 可自行調整的參數 ───────────────────────
static constexpr int   FIRE_THRESH_8U = 220;   // 0-255：亮度門檻
static constexpr double AREA_MIN_PX   = 10.0; // 最小面積 (px)
static constexpr int   CIRCLE_THICK   = 1;     // 線粗
// ───────────────────────────────────────────

// === 🔥 亮度門檻 → 找含最高溫點的輪廓 → 畫方框 ===
static void drawFireRectSeed(const cv::Mat &gray8,
                             cv::Mat       &canvas,
                             int seedX, int seedY)
{
    cv::Mat mask;
    ...
    for (auto &c : cs)
    {
        if (cv::pointPolygonTest(c, cv::Point2f(seedX, seedY), /*measureDist*/false) < 0)
            continue;                       // 最高溫不在此輪廓

        if (cv::contourArea(c) < AREA_MIN_PX) return;  // 雜訊
...
public:
    ...
    int frameCallback(guide_usb_frame_data_t *pVideoData)
    {
        ...
        // ★ 只框含最高溫點的輪廓
        drawFireRectSeed(img8, color_img, max_x, max_y);
